...
namespace chess {
    ...
    class _Position
    {
        ...
        template<Color c, bool capturesOnly = false>
        void genPawnSingleMoves(Movelist &mv) const;
        template<Color c, bool capturesOnly = false>
        inline void genKnightMoves(Movelist &list) const
        {
...
#pragma unroll
        ...
        inline void placePiece(PieceType pt, Square s, Color c) {
            ...
            current_state.pieces_list[s] = make_piece<PieceC>(pt, c);
            current_state.hash ^= RandomPiece[static_cast<int>(current_state.pieces_list[s])][s];
            current_state.kings[c] = a?s:SQ_NONE;
        }
    ...
    private:
        ...
        [[nodiscard]] inline Bitboard pinMask(Color c, Square sq) const
        {
            ...
            while (pt_attacks)
            {
                ...
                bool v = popcount(possible_pin & occ_us)==1;
    ...
    public:
      inline HistoryEntry<PieceC> state() const { return current_state; }
