...
namespace chess::zobrist {  // ahh zobrist hash
    ...
    constexpr uint64_t RandomCastle[16] = {
...
namespace chess {
    namespace _chess {
        template<typename T>
        concept PieceTypeConcept = std::same_as<T, PieceType>;
    }  // namespace _chess [INTERNAL]
    ...
    struct HistoryEntry {
        ...
        uint8_t  halfMoveClock;   // Half-move clock for 50-move rule
    ...
    class Position
    {
        ...
        template<
          ...
          std::enable_if_t<(std::is_same_v<T, PolyglotPiece> || std::is_same_v<T, EnginePiece>)
                           ...
                           int> = 0>
        [[deprecated("Incompatible piece method used, you shouldn't call this")]] inline Position(
...
#pragma unroll
        ...
        template<_chess::PieceTypeConcept... PTypes>
        [[nodiscard]] inline Bitboard pieces(PTypes... ptypes) const { return (pieces(ptypes) | ...); }
        template<_chess::PieceTypeConcept... PTypes>
        [[nodiscard]] inline Bitboard pieces(Color c, PTypes... ptypes) const { return (pieces(ptypes, c) | ...); }
