    ...
    auto do_parse = [&](std::string_view input_san) -> Move {
        ...
        if (san == "O-O" || san == "0-0" || san == "O-O+" || san == "0-0+" || san == "O-O#" || san == "0-0#") {
            ...
            if (std::find(moves.begin(), moves.end(), km) != moves.end())
                return km;
            THROW_IF_EXCEPTIONS_ON(IllegalMoveException("illegal O-O"));
            return Move::none();
        }
        ...
        if (san.empty()) {
            THROW_IF_EXCEPTIONS_ON(IllegalMoveException("empty san"));
            return Move::none();
        }
        ...
        if (san.size() >= 3) {
            ...
            if (penult == '=') {
                ...
                if (promotion == NO_PIECE_TYPE) {
                    THROW_IF_EXCEPTIONS_ON(IllegalMoveException("invalid promotion piece"));
                    return Move::none();
                }
        ...
        if (san.size() < 2)
            THROW_IF_EXCEPTIONS_ON(IllegalMoveException("invalid san (too short)"));
        char dfile = san[san.size() - 2];
        char drank = san[san.size() - 1];
        if (!(dfile >= 'a' && dfile <= 'h' && drank >= '1' && drank <= '8')) {
            THROW_IF_EXCEPTIONS_ON(IllegalMoveException("invalid destination square"));
            return Move::none();
        }
        std::string dest_sq_str = san.substr(san.size() - 2, 2);
        Square to_square = parse_square(dest_sq_str);
        if (to_square == SQ_NONE) {
            THROW_IF_EXCEPTIONS_ON(IllegalMoveException("invalid dest square"));
            return Move::none();
        }
        ...
        if (prefix.size() >= 2) {
            ...
            if (sfile >= 'a' && sfile <= 'h' && srank >= '1' && srank <= '8') {
                ...
                if (src_square == SQ_NONE)
                    THROW_IF_EXCEPTIONS_ON(IllegalMoveException("invalid source square"));
        ...
        for (char c : prefix) {
            if (c >= 'a' && c <= 'h')
                ...
                THROW_IF_EXCEPTIONS_ON(IllegalMoveException("invalid char in SAN prefix"));
                return Move::none();
            }
        }
        ...
        for (Move m : moves) {
            ...
            if (found) {
                THROW_IF_EXCEPTIONS_ON(
                    AmbiguousMoveException("ambiguous san: \"" + std::string(raw_san) + "\" in " + pos.fen()));
                return Move::none();
            }
            matched = m;
            found = true;
        ...
        if (!found) {
            THROW_IF_EXCEPTIONS_ON(IllegalMoveException("illegal san: \"" + std::string(input_san) + "\" in " + pos.fen()));
            return Move::none();
        }
